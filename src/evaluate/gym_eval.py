...
class RuntimeActionNet:
    ...
    def act(self, data_packet: Any) -> int:
        ...
        # 4 RGB frames concatenated along channels
        # [12, 84, 84] -> [1, 12, 84, 84]
        if state.ndim == 3:
        ...
        if state.ndim != 4:
            raise ValueError(
                "ActionNet expected input with shape "
                "[batch, 12, height, width], but received "
                f"{tuple(state.shape)}."
            )

        if state.shape[1] != 12:
            raise ValueError(
                "ActionNet expected four stacked RGB frames "
                "(12 channels total), but received "
        ...
        with torch.inference_mode():
            ...
            # Prediction corresponding to newest frame.
...
class RuntimeActionNet:
    ...
    def act(
        ...
        if logits.ndim != 3:
            raise RuntimeError(
                f"Expected ActionNet output [B, 4, 6], got {tuple(logits.shape)}"
            )

        if logits.shape[1] != 4:
            raise RuntimeError(
                f"Expected four temporal predictions, got {tuple(logits.shape)}"
            )
        ...
        newest_logits = logits[:, -1, :]

        action = int(newest_logits.argmax(dim=-1).item())

        # -----------------------------------------------------
        # IMPORTANT:
        #
        # NO map_canonical_actions().
        #
        # Class ID IS the Space Invaders Gym action ID.
        # -----------------------------------------------------
